...
export interface CoordinationTeamAskRecord {
  ...
  synthesized?: boolean;
  /** Set when synthesis failed with a user-actionable error (e.g. billing, invalid key). */
  synthesisWarning?: string;
  /** Set to true when the operator explicitly dismisses the card. Dismissed asks are not carried forward. */
  dismissed?: boolean;
  updatedAt: string;
}
...
export interface CoordinationStateRecord {
  ...
  /** Cumulative token usage for the Haiku coordination synthesizer in this workspace. */
  coordinatorUsage?: { inputTokens: number; outputTokens: number; costUsd: number; callCount: number };
}
